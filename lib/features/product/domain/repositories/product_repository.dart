...
class ProductRepository implements ProductRepositoryInterface {
  ...
  Future<ApiResponse> getFilteredProductList(BuildContext context,
    ...
    } else if (productType == ProductType.latestProduct) {
      endUrl = AppConstants.latestProductUri;
      title = getTranslated('latest_product', context);
    }
    try {
  ...
  Future<ApiResponse> getBrandOrCategoryProductList(
      {required bool isBrand, required int id, required int offset}) async {
    try {
      String uri;
      if (isBrand) {
        uri =
  ...
  Future<ApiResponse> getClearanceAllProductList(String offset) async {
    try {
      final response =
          await dioClient!.get(AppConstants.clearanceAllProductUri + offset);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
