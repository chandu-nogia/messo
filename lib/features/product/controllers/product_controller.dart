...
import 'package:flutter_lovexa_ecommerce/features/category/domain/models/find_what_you_need.dart';
import 'package:flutter_lovexa_ecommerce/features/product/domain/models/home_category_product_model.dart';
import 'package:flutter_lovexa_ecommerce/features/product/domain/models/most_demanded_product_model.dart';
...
class ProductController extends ChangeNotifier {
  ...
  List<Product>? _bestSellingProductList = [];
  ...
  String? _title = '${getTranslated('best_selling', GetCtx.context!)}';
    ...
    if (!_offsetList.contains(offset)) {
      ...
      ApiResponse apiResponse = await productServiceInterface!
          .getFilteredProductList(
              GetCtx.context!, offset.toString(), _productType, title);
  ...
  Future<List<Product>> getLatestProductListApis(int offset,
      {bool reload = false}) async {
    String endUrl = AppConstants.latestProductUri;
    ...
    if (!_offsetList.contains(offset)) {
      _offsetList.add(offset);
      ApiResponse apiResponse = await productServiceInterface!
          .getFilteredProductList(GetCtx.context!, offset.toString(),
              ProductType.latestProduct, title);
      print("latest data res :: ${apiResponse.response!.data}");

      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        featureFn(apiResponse);
        if (offset == 1) {
          _latestProductList = [];
        }
        if (ProductModel.fromJson(apiResponse.response!.data).products !=
    ...
    return _latestProductList!;
  }

  Future<List<Product>> getBestSellingProductListApis(int offset,
      {bool reload = false}) async {
    String endUrl = AppConstants.latestProductUri;
    ...
    if (localData != null &&
        ...
        ProductModel.fromJson(jsonDecode(localData.response)).products !=
            null) {
      _bestSellingProductList = [];
      _bestSellingProductList!.addAll(
          ProductModel.fromJson(jsonDecode(localData.response)).products!);
      _latestPageSize =
          ProductModel.fromJson(jsonDecode(localData.response)).totalSize;
      _filterFirstLoading = false;
  ...
  updateProduct(ProductModel product) {
    _brandOrCategoryProductList = product;
    notifyListeners();
  }
  ...
  Future<void> initBrandOrCategoryProductList(
    ...
    if (apiResponse.response?.statusCode == 200) {
      print("data add ::: ${isBrand}, ${id}");
      print(
          "brand or category product list ::::::::: ${apiResponse.response?.data}");
      if (offset == 1) {
...
//! feature product
  ...
  Future<List<Product>> getFeaturedProductListApis(String offset,
      {bool reload = false}) async {
    var localData =
        await database.getCacheResponseById(AppConstants.featuredProductUri);
    ...
    if (!_featuredOffsetList.contains(offset)) {
      ...
      ApiResponse apiResponse =
          await productServiceInterface!.getFeaturedProductList(offset);
      print("featured product list ::::::::: ${apiResponse.response?.data}");
      if (apiResponse.response != null &&
          apiResponse.response!.statusCode == 200) {
        featureFn(apiResponse);
  ...
  Future<void> getDiscountedProductList(int offset, bool reload,
    ...
    print("product discount ::: ${apiResponse.response!.data}");
