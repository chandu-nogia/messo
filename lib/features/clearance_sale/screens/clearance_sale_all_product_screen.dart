...
import 'package:flutter_lovexa_ecommerce/features/clearance_sale/widgets/clearance_search_bar_widget.dart';
...
class ClearanceSaleAllProductScreen extends StatefulWidget {
  ...
  @override
  State<ClearanceSaleAllProductScreen> createState() =>
      _ClearanceSaleAllProductScreenState();
}

class _ClearanceSaleAllProductScreenState
    extends State<ClearanceSaleAllProductScreen> {
  ...
  void initState() {
    ...
    Provider.of<ProductController>(GetCtx.context!, listen: false)
        .getClearanceSearchProduct(offset: 1, query: '', isNotify: false);
  }
                ...
                child: Consumer<ProductController>(
                  ...
                  return ClearanceSearchBarWidget(
                      ...
                      onSearchClick: () => onClick(
                          context, productController, _searchController),
                      searchIcon: Icon(
                ...
                child: SingleChildScrollView(
                  ...
                  child: Consumer<ProductController>(
                    ...
                    ProductModel? productModel =
                        productController.clearanceSearchProductModel;

                    return (productModel != null &&
                            !productController.isSearchLoading)
                        ? (productModel.products != null &&
                                productModel.products!.isNotEmpty)
                            ? PaginatedListView(
                                scrollController: _scrollController,
                                onPaginate: (int? offset) async => applyFilter(
                                    context: context,
                                    fromPagination: true,
                                    offset: offset),
...
void applyFilter(
    {required BuildContext context, bool fromPagination = false, int? offset}) {
  ...
  if (fromPagination) {
    productController.getClearanceSearchProduct(
        query: productController.isSearchActive
            ? productController.searchText ?? ''
            : '',
...
void onClick(BuildContext context, ProductController productController,
  ...
  if (searchController.text.isEmpty && !productController.isSearchActive) {
    showCustomSnackBar(
        ...
        context);
  } else if (!productController.isSearchActive &&
      !productController.isSearchLoading) {
    productController.setSearchText(searchController.text.trim());
    productController.getClearanceSearchProduct(
        offset: 1, query: productController.searchText!);
