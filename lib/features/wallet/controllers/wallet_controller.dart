...
import 'package:flutter_lovexa_ecommerce/features/wallet/domain/models/wallet_bonus_model.dart';
...
class WalletController extends ChangeNotifier {
  ...
  Future<void> addFundToWallet(String amount, String paymentMethod) async {
    ...
    if (apiResponse.response != null &&
      ...
      showCustomSnackBar(
          "Minimum= ${PriceConverter.convertPrice(GetCtx.context!, double.tryParse('${apiResponse.response?.data['minimum_amount']}'))} and Maximum=${PriceConverter.convertPrice(GetCtx.context!, double.tryParse('${apiResponse.response?.data['maximum_amount']}'))}",
          GetCtx.context!);
    } else {
      _isConvert = false;
      ApiChecker.checkApi(apiResponse);
