...
class PaymentScreenState extends State<AddFundToWalletScreen> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      canPop: false,
      onPopInvokedWithResult: (val, canExecute) =>
          canExecute == true ? null : _exitApp(context),
      child: Scaffold(
          backgroundColor:
    ...
    Future.delayed(const Duration(milliseconds: 100)).then((_) {
      ...
      showAnimatedDialog(
          ...
          OrderPlaceDialogWidget(
            icon: Icons.clear,
            title: getTranslated('payment_cancelled', GetCtx.context!),
            description:
                getTranslated('your_payment_cancelled', GetCtx.context!),
            isFailed: true,
          ),
          dismissible: false,
          willFlip: true);
