...
import 'package:flutter_lovexa_ecommerce/features/location/domain/models/place_details_model.dart';
import 'package:flutter_lovexa_ecommerce/features/location/domain/models/prediction_model.dart';
...
class LocationController with ChangeNotifier {
  ...
  void getCurrentLocation(BuildContext context, bool fromAddress,
    ...
    try {
      String address = await getAddressFromGeocode(
          LatLng(myPosition.latitude, myPosition.longitude), GetCtx.context!);
      myPlaceMark =
  ...
  Future<String> getAddressFromGeocode(
    ...
    if (response.response!.statusCode == 200 &&
      ...
      // address =
      List addr = response.response!.data['results'][0]['formatted_address']
          .toString()
          .split(',')
          .toList();
      addr.removeRange(addr.length - 3, addr.length);
      address = addr.join(',');
      ...
      // String? state;
      // String? pincode;
      // var cityss = response.response!.data['plus_code']['compound_code']
      //     .toString()
      //     .split(" ")
      //     .removeAt(1)
      //     .toString();
      // _cityController.text = cityss;
      // print("citySS:::: $cityss");
      ...
      for (var component in addressComponents) {
        ...
        if (types.contains("administrative_area_level_3")) {
          ...
          _cityController.text = component["long_name"]; // City
        }
        if (types.contains("administrative_area_level_1")) {
          _stateController.text = component["long_name"]; // State
        }
        if (types.contains("postal_code")) {
          _zipCodeController.text = component["long_name"]; // Pincode
        }
      }
