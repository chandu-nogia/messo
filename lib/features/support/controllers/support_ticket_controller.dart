import 'package:flutter_lovexa_ecommerce/features/support/domain/models/support_reply_model.dart';
import 'package:flutter_lovexa_ecommerce/features/support/domain/models/support_ticket_body.dart';
...
class SupportTicketController extends ChangeNotifier {
  ...
  Future<http.StreamedResponse> createSupportTicket(
    ...
    final data = supportTicketBody.toJson();
    print(
        ":::: ${await response.stream.bytesToString()} \n $data  \n ${response.statusCode}  \n ${response.request!.url}");
    if (response.statusCode == 200) {
      showCustomSnackBar(
          '${getTranslated('support_ticket_created_successfully', GetCtx.context!)}',
          GetCtx.context!,
          isError: false);
  ...
  Future<http.StreamedResponse> sendReply(int? ticketID, String message) async {
    ...
    if (response.statusCode == 200) {
      getSupportTicketReplyList(GetCtx.context!, ticketID);
      _pickedImageFiles = [];
      pickedImageFileStored = [];
      _isLoading = false;
    } else {
  ...
  Future<void> closeSupportTicket(int? ticketID) async {
    ...
    if (apiResponse.response != null &&
      ...
      getSupportTicketList();
      showCustomSnackBar(
          '${getTranslated('ticket_closed_successfully', GetCtx.context!)}',
          GetCtx.context!,
          isError: false);
    } else {
      ApiChecker.checkApi(apiResponse);
  ...
  String selectedPriority =
      getTranslated('select_priority', GetCtx.context!) ?? '';
  String selectedPriorityEn = '';

  void setInitialSelectedPriority() {
    selectedPriority = getTranslated('select_priority', GetCtx.context!) ?? '';
  ...
  void setSelectedPriority(int index, {bool reload = true}) {
    ...
    selectedPriority =
        getTranslated(priority[selectedPriorityIndex], GetCtx.context!) ??
            'High';
