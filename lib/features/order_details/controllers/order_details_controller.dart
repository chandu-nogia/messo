...
import 'package:flutter_lovexa_ecommerce/features/order_details/domain/models/order_details_model.dart';
...
class OrderDetailsController with ChangeNotifier {
  ...
  //   print("order details :: ${apiResponse.response!.data}");
  ...
  Future<ApiResponse> getOrderDetails(String orderID) async {
    ...
    if (apiResponse.response?.statusCode == 200 &&
      ...
      final List<dynamic> data = apiResponse.response!.data;

      _orderDetails =
          data.map((order) => OrderDetailsModel.fromJson(order)).toList();

      print("Parsed OrderDetails length: ${_orderDetails?.length}");
    } else {
      print("Failed to load order details or empty response.");
      _orderDetails = [];
    }
  ...
  Future<ApiResponse> downloadDigitalProduct({int? orderDetailsId}) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      Provider.of<AuthController>(GetCtx.context!, listen: false).resendTime =
  ...
  void productDownload(
    ...
    if (isIos) {
      ...
      if (apiResponse.statusCode == 200) {
        ...
        if (fileExists) {
          ScaffoldMessenger.of(GetCtx.context!).showSnackBar(
          ...
          showCustomSnackBar(
              getTranslated('product_downloaded_successfully', GetCtx.context!),
              GetCtx.context!,
              isError: false);
        }
        ...
        _isDownloaodLoading = false;
        showCustomSnackBar(
            getTranslated('product_download_failed', GetCtx.context!),
            GetCtx.context!);
        Navigator.of(GetCtx.context!).pop();
      }
    } else {
      ...
      if (fileExists) {
        showCustomSnackBar(
            getTranslated('file_already_downloaded', GetCtx.context!),
            GetCtx.context!);
      } else {
