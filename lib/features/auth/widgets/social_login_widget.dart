...
import 'package:flutter_lovexa_ecommerce/features/auth/domain/models/signup_model.dart';
import 'package:flutter_lovexa_ecommerce/features/auth/domain/models/social_login_model.dart';
import 'package:flutter_lovexa_ecommerce/features/auth/enums/from_page.dart';
import 'package:flutter_lovexa_ecommerce/features/auth/enums/social_login_options.dart';
import 'package:flutter_lovexa_ecommerce/features/auth/screens/otp_registration_screen.dart';
import 'package:flutter_lovexa_ecommerce/features/auth/widgets/existing_account_bottom_sheet.dart';
import 'package:flutter_lovexa_ecommerce/features/profile/domain/models/profile_model.dart';
...
class SocialLoginWidgetState extends State<SocialLoginWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    //               if(context.mounted){}
    //               if(Provider.of<GoogleSignInController>(GetCtx.context!,listen: false).googleAccount != null){
    //                 id = Provider.of<GoogleSignInController>(GetCtx.context!,listen: false).googleAccount!.id;
    //                 email = Provider.of<GoogleSignInController>(GetCtx.context!,listen: false).googleAccount!.email;
    //                 token = Provider.of<GoogleSignInController>(GetCtx.context!,listen: false).auth.accessToken;
    //                 medium = 'google';
    ...
    //                 await Provider.of<AuthController>(GetCtx.context!, listen: false).socialLogin(socialLogin, route);
    //               }
    //
    //             },
    ...
    //               String? id,token,email, medium;
    //               if(Provider.of<FacebookLoginController>(GetCtx.context!,listen: false).userData != null){
    //                 id = Provider.of<FacebookLoginController>(GetCtx.context!,listen: false).result.accessToken!.userId;
    //                 email = Provider.of<FacebookLoginController>(GetCtx.context!,listen: false).userData!['email'];
    //                 token = Provider.of<FacebookLoginController>(GetCtx.context!,listen: false).result.accessToken!.token;
    //                 medium = 'facebook';
    //                 socialLogin.email = email;
    //                 socialLogin.medium = medium;
    //                 socialLogin.token = token;
  ...
  if (isRoute) {
    if (phone != null) {
      ...
      Navigator.pushAndRemoveUntil(
          ...
          MaterialPageRoute(
              builder: (_) => OtpRegistrationScreen(
                  ...
                  userInput: Provider.of<GoogleSignInController>(
                              GetCtx.context!,
                              listen: false)
                          .googleAccount
                          ?.email ??
                      '',
                  userName: Provider.of<GoogleSignInController>(GetCtx.context!,
                              listen: false)
                          .googleAccount
                          ?.displayName ??
                      '')),
      ...
      showModalBottomSheet(
          ...
          backgroundColor:
              Theme.of(GetCtx.context!).primaryColor.withValues(alpha: 0),
      ...
      showCustomSnackBar(errorMessage, GetCtx.context!);
    }
  } else {
    showCustomSnackBar(errorMessage, GetCtx.context!);
  }
...
Future<void> googleLogin(BuildContext context) async {
  ...
  try {
    ...
    String? id, token, email, medium;
    if (context.mounted) {}
    if (Provider.of<GoogleSignInController>(GetCtx.context!, listen: false)
            .googleAccount !=
        null) {
      id = Provider.of<GoogleSignInController>(GetCtx.context!, listen: false)
          .googleAccount!
          .id;
      email =
          Provider.of<GoogleSignInController>(GetCtx.context!, listen: false)
              .googleAccount!
              .email;
      token =
          Provider.of<GoogleSignInController>(GetCtx.context!, listen: false)
...
Future<void> facebookLogin(BuildContext context) async {
  ...
  try {
    await Provider.of<FacebookLoginController>(context, listen: false).login();
    String? id, token, email, medium;
    if (Provider.of<FacebookLoginController>(GetCtx.context!, listen: false)
            .userData !=
        null) {
      id = Provider.of<FacebookLoginController>(GetCtx.context!, listen: false)
          .result
          .accessToken!
          .userId;
      email =
          Provider.of<FacebookLoginController>(GetCtx.context!, listen: false)
              .userData!['email'];
      token =
          Provider.of<FacebookLoginController>(GetCtx.context!, listen: false)
              .result
      ...
      socialLogin.token = token;
      socialLogin.uniqueId = id;
      await Provider.of<AuthController>(GetCtx.context!, listen: false)
          .socialLogin(socialLogin, route);
    }
  } catch (er) {
    debugPrint('access token error is : $er');
