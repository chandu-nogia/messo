...
import 'package:flutter_lovexa_ecommerce/features/checkout/screens/checkout_screen.dart';
import 'package:flutter_lovexa_ecommerce/features/product/domain/models/product_model.dart';
import 'package:flutter_lovexa_ecommerce/features/product_details/widgets/color_selection_widget.dart';
...
import 'package:flutter_lovexa_ecommerce/features/product_details/widgets/shipping_method_dialog.dart';
import 'package:flutter_lovexa_ecommerce/features/shipping/domain/models/shipping_method_model.dart';
...
class CartBottomSheetWidgetState extends State<CartBottomSheetWidget> {
          ...
          child: Consumer<ProductDetailsController>(
            builder: (ctx, details, child) {
              ...
              if (widget.product?.productType == 'physical' &&
                  ...
                  widget.product?.isRestockRequested == 1) {
                variationRestockRequested = widget
                    .product!.restockRequestedList!
                    .contains(variationType);
              } else {
                                    ...
                                    child: Column(
                                      ...
                                      Column(
                                          ...
                                          children: [
                                            CustomDirectionalityWidget(
                                              child: Text(
                                                  '${startingPrice != null ? PriceConverter.convertPrice(context, startingPrice, discount: (widget.product?.clearanceSale?.discountAmount ?? 0) > 0 ? widget.product?.clearanceSale?.discountAmount : widget.product?.discount, discountType: (widget.product?.clearanceSale?.discountAmount ?? 0) > 0 ? widget.product?.clearanceSale?.discountType : widget.product?.discountType) : ''}'
                                                  '${endingPrice != null ? ' - ${PriceConverter.convertPrice(context, endingPrice, discount: (widget.product?.clearanceSale?.discountAmount ?? 0) > 0 ? widget.product?.clearanceSale?.discountAmount : widget.product?.discount, discountType: widget.product?.discountType)}' : ''}',
                                                  style: titilliumRegular.copyWith(
                                                      color: Theme.of(context)
                                                          .primaryColor,
                                                      fontSize: Dimensions
                        ...
                        ? Provider.of<AuthController>(context, listen: false)
                                  ...
                                  buttonText:
                                      (widget.product!.choiceOptions!.isEmpty
                                              ? widget.product!
                                                      .isRestockRequested ==
                                                  1
                                              : variationRestockRequested)
                                          ? getTranslated(
                                              'restock_requested', context)
                                          : getTranslated(
                                              'request_restock', context),
                                  onTap: (widget.product!.choiceOptions!.isEmpty
                                          ? widget.product!
                                                  .isRestockRequested ==
                                              1
                                          : variationRestockRequested)
                            ...
                            : Padding(
                                ...
                                child: CustomButton(
                                  ...
                                  buttonText:
                                      getTranslated('request_restock', context),
                                  onTap: () {
                                    Navigator.of(context).pop();
                                    showCustomSnackBar(
                                        getTranslated('to_request_the_restock',
                                            GetCtx.context!),
                                        GetCtx.context!);
                                  },
                                ),
  ...
  void _navigateToCheckoutScreen(
    ...
    print("quantity::: ${totalQuantity}  :: Price :: ${amount}");
    ...
    Navigator.of(GetCtx.context!).push(MaterialPageRoute(
