...
import 'package:flutter_lovexa_ecommerce/features/coupon/domain/models/coupon_item_model.dart';
import 'package:flutter_lovexa_ecommerce/features/coupon/domain/models/coupon_model.dart';
...
class CouponController extends ChangeNotifier {
  ...
  Future<void> applyCoupon(
    ...
    if (apiResponse.response != null &&
      ...
      showCustomSnackBar(
          '${getTranslated('you_got', GetCtx.context!)} '
          '${PriceConverter.convertPrice(GetCtx.context!, _discount)} '
          '${getTranslated('discount', GetCtx.context!)}',
          GetCtx.context!,
          isError: false,
          isToaster: true);
    } else {
      showCustomSnackBar(apiResponse.response?.data, GetCtx.context!,
          isToaster: true);
    }
    _isLoading = false;
    notifyListeners();
  ...
  Future<void> getCouponList(BuildContext context, int offset) async {
    ...
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      print("coupen list ::: =>:: ${apiResponse.response!.data}");
      couponList = [];
      _isLoading = false;
      couponList!.addAll(
  ...
  Future getAvailableCouponList() async {
    try {
      ...
      if (apiResponse.response != null &&
        ...
        if (apiResponse.response?.data[0] is Map) {
          apiResponse.response?.data.forEach(
              (coupon) => availableCouponList?.add(Coupons.fromJson(coupon)));
        } else {
          apiResponse.response?.data.forEach((key, coupon) =>
              availableCouponList?.add(Coupons.fromJson(coupon)));
        }
        // apiResponse.response?.data.forEach((key, coupon) =>
        //     availableCouponList?.add(Coupons.fromJson(coupon)));
      }
