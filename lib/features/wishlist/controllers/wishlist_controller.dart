...
import 'package:flutter_lovexa_ecommerce/features/wishlist/domain/models/wishlist_model.dart';
...
class WishListController extends ChangeNotifier {
  ...
  void addWishList(int? productID) async {
    ...
    if (apiResponse.response != null &&
      ...
      getWishList();
      showCustomSnackBar(
          "${getTranslated("successfully_added_to_wishlist", GetCtx.context!)}",
          GetCtx.context!,
          isError: false);
    } else {
  ...
  void removeWishList(int? productID, {int? index}) async {
    ...
    if (apiResponse.response != null &&
      ...
      getWishList();
      showCustomSnackBar(
          "${getTranslated("successfully_removed_from_wishlist", GetCtx.context!)}",
          GetCtx.context!,
          isError: false);
    } else {
      showCustomSnackBar(apiResponse.error.toString(), GetCtx.context!);
