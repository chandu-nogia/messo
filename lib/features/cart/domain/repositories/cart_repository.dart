...
class CartRepository implements CartRepositoryInterface<ApiResponse> {
  ...
  Future<ApiResponse> getList({int? offset}) async {
    try {
      final response = await dioClient!.get(
          '${AppConstants.getCartDataUri}?guest_id=${Provider.of<AuthController>(GetCtx.context!, listen: false).getGuestToken()}');
  ...
  @override
  Future<ApiResponse> restockRequest(
      CartModelBody cart,
      List<ChoiceOptions> choiceOptions,
      List<int>? variationIndexes,
      int? buyNow,
    ...
    try {
      final response =
          await dioClient!.post(AppConstants.productRestockRequest, data: data);
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
