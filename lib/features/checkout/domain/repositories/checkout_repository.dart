...
import 'package:flutter_lovexa_ecommerce/data/datasource/remote/exception/api_error_handler.dart';
...
class CheckoutRepository implements CheckoutRepositoryInterface {
  ...
  Future<ApiResponse> cashOnDeliveryPlaceOrder({
    ...
    try {
      ...
      final Map<String, dynamic> queryParams = {
        ...
        'guest_id': Provider.of<AuthController>(GetCtx.context!, listen: false)
            .getGuestToken(),
        'is_guest':
            '${Provider.of<AuthController>(GetCtx.context!, listen: false).isLoggedIn() ? 0 : 1}',
    ...
    try {
      ...
      fields.addAll(<String, String>{
        ...
        'guest_id': Provider.of<AuthController>(GetCtx.context!, listen: false)
                .getGuestToken() ??
            '',
        'is_guest': Provider.of<AuthController>(GetCtx.context!, listen: false)
                .isLoggedIn()
            ? '0'
            : '1',
  ...
  Future<ApiResponse> walletPaymentPlaceOrder(
    ...
    try {
      final response = await dioClient!.get(
        '${AppConstants.walletPayment}?address_id=$addressID&coupon_code=$couponCode&coupon_discount=$couponDiscountAmount&billing_address_id=$billingAddressId&order_note=$orderNote&guest_id=${Provider.of<AuthController>(GetCtx.context!, listen: false).getGuestToken()}&is_guest=${Provider.of<AuthController>(GetCtx.context!, listen: false).isLoggedIn() ? 0 : 1}&is_check_create_account=$isCheckAccount&password=$password',
      );
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
  ...
  Future<ApiResponse> offlinePaymentList() async {
    try {
      final response = await dioClient!.get(
          '${AppConstants.offlinePaymentList}?guest_id=${Provider.of<AuthController>(GetCtx.context!, listen: false).getGuestToken()}&is_guest=${!Provider.of<AuthController>(GetCtx.context!, listen: false).isLoggedIn()}');
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
